    ...
    <v-card elevation="5">
      ...
      <v-card-text class="text-center">
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          ></v-text-field>
          <v-btn color="primary" @click="solicitarEntregaResultados()" small>Solicitar</v-btn>
        </v-form>
      </v-card-text>
    </v-card>
  </v-container>
...
export default {
  ...
  computed: {
    ...mapState(["imageAgent", "emailToResultados"]),
    fechaMaxima() {
      return moment(new Date()).format("YYYY-MM-DD").toString();
    },
  },
  methods: {
    ...mapActions(["sendMessageUser", "sendEmail"]),
    formatearFecha(date) {
      return moment(date).locale("es").format("dddd D MMMM [de] YYYY");
    },
    solicitarEntregaResultados() {
      ...
      `;
      this.sendEmail({
        to: this.emailToResultados,
        message: datosEmail,
        subject: "Solicitud Entrega Resultados a Domicilio - Angela",
      });
    },
  },
};
