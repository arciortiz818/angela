    ...
    <v-card elevation="5">
      ...
      <v-card-text class="text-center">
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-btn color="primary" @click="solicitarCita()" small>Solicitar</v-btn>
        </v-form>
      </v-card-text>
    </v-card>
  </v-container>
...
export default {
  ...
  computed: {
    ...mapState(["imageAgent", "emailToCitas"]),
  },
  methods: {
    ...
    solicitarCita() {
      ...
      const datosEmail = `
	     ...
	     `;
      this.sendEmail({
        to: this.emailToCitas,
        message: datosEmail,
        subject: "Solicitud de Cita - Angela",
      });
    },
  },
};
